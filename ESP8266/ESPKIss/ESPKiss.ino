...
int TXDELAY = 100;		// Number of flags
...
void setup()
{
	...
	// initialise timer0 to generate RX interrupts at bit frequency

	noInterrupts();
	timer0_isr_init();
	timer0_attachInterrupt(timer0_rx_handler);
	nexttonetick = ESP.getCycleCount() + BIT_DELAY;
...
void loop()
{
	if (MSGREADY && transmitting == 0)
	{
		...
		noInterrupts();
		timer0_attachInterrupt(timer0_tx_handler);
...
//	We look for changes in tone by timing zero crossings on the input. If within certain 
//	limits. set low or high, if not assume noise. Once we have found tone changes, we set 
//	bit clock to sample alf way through nominal bit position.

//	I'm sure this is easier said than done!

//	I'm not sure how much of this can be done in background. A pin change interrupt 
//	on zero crossing would be nice, but not sure how to tie to ADC (or could just clip
//	audio and feed to digital input.
...
void ICACHE_RAM_ATTR timer0_tx_handler ()
{
