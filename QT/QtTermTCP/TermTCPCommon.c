...
{
	int MonLen = 0, wlen, err = 0, newlen;
	unsigned char * ptr;
...
/*
		...
		sprintf(MsgFile, "%s%s", YAPPPath, WFN);
		...
		len = sprintf(BufferW, "YAPP Receving File %s size %d\r", WFN, FileSize);
		WritetoOutputWindow(BufferW, len);
		DoRefresh(&OutputData);
	...
	case STX:
		...
		if (YAPPDate)
		{
			...
			if (Sum != *uptr)
			{
				...
				len = sprintf(BufferW, "YAPPC Checksum Error\r", MsgFile);
				WritetoOutputWindow(BufferW, len);
		...
		if ((YAPPLen)+pktLen > MailBufferSize)
		{
			...
			len = sprintf(BufferW, "YAPP Too much data received\r", MsgFile);
			WritetoOutputWindow(BufferW, len);
...
//					struct timeval times[2];
			...
			if (Written != YAPPLen)
			{
				...
				len = sprintf(BufferW, "Failed to save YAPP File\r", MsgFile);
				WritetoOutputWindow(BufferW, len);
...
void YAPPSendFile(WCHAR * FN)
{
	...
	if (MsgFile == NULL)
	{
		Len = sprintf(BufferW, "Filename missing\r");
		WritetoOutputWindow(BufferW, Len);
...
*/
	...
	Len = sprintf(BufferW, "File %s not found\r", FN);
	WritetoOutputWindow(BufferW, Len);
