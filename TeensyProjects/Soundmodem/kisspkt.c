...
// 11 is actual RX level, 12 can't be used (ACKMode)
#define  KISS_CMD_CENTREFREQ 13
...
void pkttransmitloop(struct state *state)
{
	...
	if (chan->pkt.htx.rd != chan->pkt.htx.wr)
	{
		...
		{
			if (globaldcd(state))
				return;					// wait for channel to clear

			if ((random_num() & 0xff) <= state->chacc.ppersist)
			{
				// Lost CSMA try again after slot time

				txSleep(state->chacc.slottime);
				return;
			}
		}
