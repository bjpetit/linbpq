...
static int rxfilter_real(const int16_t *val, unsigned int phase)
{
...
static int rxfilter_imag(const int16_t *val, unsigned int phase)
{
...
extern int calcsync(int *toten, int *corren, cplxshort_t *samples)
{
...
//	if (acc2 > 10)
//		WriteDebugLog(258, "Sync energy %d correlation %d\n", acc2, acc1r);
	
	if (acc2 < 16*TRAINSYMS || acc1r*2 < acc2)
		return 0;
	WriteDebugLog(257, "Sync found, energy %d correlation %d\n", acc2, acc1r);
	return 1;
}
			...
			if (calcsync(&toten1, &syncen1, syncbuf+i)) {
				if (!calcsync(&toten2, &syncen2, syncbuf+i+1) || syncen2 < syncen1) {
					...
					if (syncen2 > syncen1 && syncen2 > syncen3)
						...
						phase += (phaseinc >> 1);
					WriteDebugLog(256, "Sync found, dcd %u\n", dcdcnt);
					return phase;
				}
			}
			if (dcdcnt > 0) {
...
static void symtochar(unsigned char *sym, unsigned char *msg)
{
	unsigned int i, j, k;
...
static void mlseblock(struct rxstate *rx, unsigned int phase, unsigned int phaseinc, cplxshort_t *channel)
{
	...
	for (i = DATASYMS+CHANNELLEN-1; i > 0;) {
		...
		for (k = 0; k < j; k++) {
//			WriteDebugLog(260, "RxSymbol: %d%+di\n", samp[k].re, samp[k].im);
	...
	symtochar(sym, msg);
	if (k != MLSEROOTNODE)
		WriteDebugLog(1, "MLSE: uhoh surviving path does not end in root node 0x%x, 0x%x\n", MLSEROOTNODE, k);
#if 0
	j = 0;
	for (i = 1; i < MLSENODES; i++)
		if (nptr1[i] < nptr1[j])
			j = i;
	if (j != MLSETOORNODE)
		WriteDebugLog(1, "MLSE: uhoh best metric not at toor node 0x%x, 0x%x (en %u, %u)\n", 
...
//		k <<= 2;
//#endif /* USEVIS */
//	WriteDebugLog(7, "MLSE: signal energy %u, error energy %u  S/(N+D) %5.1fdB\n",
...
static void receiver(void *__rx)
{
	...
	WriteDebugLog(256, "rxphaseinc: 0x%x\n", phaseinc);
	for (;;) {
		...
		if (abs(splphasediff) < 0x40000) {
			...
			WriteDebugLog(257, "last train phase: 0x%08x  current train phase: 0x%08x  diff: %d\n",
				  oldphase, phase, splphasediff);
			WriteDebugLog(258, "channel angle: 0x%04x  prev channel angle: 0x%04x  diff: %d\n", changle, oldchangle, anglediff);
			WriteDebugLog(257, "total angle difference: %6d  total phase difference: %8d\n", totanglediff, totphasediff);
			WriteDebugLog(257, "nominal phase inc: 0x%05x  estimated phase inc: 0x%05x  delta: %3d o/oo\n",
...
void DemodPSK(short * buffer, int count)
{
