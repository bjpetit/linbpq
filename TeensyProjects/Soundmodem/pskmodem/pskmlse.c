...
extern void simdtrellis(unsigned int *nodemetric1, unsigned int *nodemetric2, unsigned int *metrictab, unsigned short *backptr, int vr, int vi)
{
}
...
static unsigned int metric(struct metrictab *mtab, int vr, int vi)
{
        int x, y;
        x = vr - mtab->re;
        y = vi - mtab->im;
		...
		for (i2 = 0; i2 < 2; i2++) 
			for (i3 = 0; i3 < (1<<(SYMBITS)); i3++) {
				...
				if (abs(vr) > 32767 || abs(vi) > 32767)
					WriteDebugLog(0, "initmetric: metric overflow table %u value %d%+di\n", i, vr, vi);
...
extern void simdinitmetric(const cplxshort_t *channel, metrictab_t *metrictab)
{
}
