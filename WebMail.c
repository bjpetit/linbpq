...
char WebMailSignon[] = "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">"
	...
	"<style>body { font-family: Arial, sans-serif; margin: 20px; background: #f6f7f8; color: #1f2937; } h2, h3 { text-align: center; } .form-container { max-width: 400px; margin: 20px auto; padding: 20px; border: 1px solid #ccc; border-radius: 6px; background: #fff; } "
	".form-row { margin: 15px 0; } label { display: block; margin-bottom: 5px; } input[type=text], input[type=password] { width: 100%%; padding: 8px; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; } .btn { display: inline-flex; align-items: center; justify-content: center;"
	"min-height: 44px; padding: 10px 16px; background: #fff; text-decoration: none; border-radius: 6px; border: 1px solid #ccc; color: #1f2937; box-sizing: border-box; font-size: 15px; cursor: pointer; margin-right: 10px; margin-top: 10px; } .btn:hover { background: #e9ecef; } "
	".btn:active { background: black; color: white; }</style>"
	"</head><body>"
...
static char MsgInputPage[] = "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><meta content=\"text/html; charset=UTF-8\" http-equiv=\"content-type\">"
	...
	"<style>body { font-family: Arial, sans-serif; margin: 12px; background: #f5f7fa; color: #1f2937; } h3 { text-align: center; margin: 0 0 12px; } #myform { max-width: 980px; margin: 0 auto; background: #fff; border: 1px solid #d1d5db; "
	"border-radius: 8px; padding: 14px; box-sizing: border-box; } .form-row { display: flex; align-items: center; gap: 10px; margin: 8px 0; } .form-row label { flex: 0 0 88px; font-weight: 600; } .form-row input, .form-row select { flex: 1 1 auto; "
	"min-width: 0; padding: 8px 10px; border: 1px solid #cbd5e1; border-radius: 6px; box-sizing: border-box; font-size: 14px; } #To, #Subj { font-size: 15px; padding-top: 9px; padding-bottom: 9px; } .form-row input[type=file] { padding: 6px 0; border: none; }"
	".form-row textarea { flex: 1 1 auto; width: 100%%; min-height: 360px; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; box-sizing: border-box; line-height: 1.4; resize: vertical; } .form-row.message-row { align-items: flex-start; } "
	".form-row.message-row label { padding-top: 10px; } .form-row.meta-row label { flex: 0 0 88px; } .form-row.meta-row select { flex: 0 0 120px; max-width: 120px; } .form-row.meta-row input { flex: 0 1 260px; max-width: 260px; } "
	".inline-action { white-space: nowrap; flex: 0 0 auto; margin-left: 10px; } .btn, .buttons input { display: inline-flex; align-items: center; justify-content: center; min-height: 38px; padding: 8px 14px; border: 1px solid #94a3b8; "
	"border-radius: 6px; background: #fff; color: #1f2937; cursor: pointer; text-decoration: none; font-size: 14px; box-sizing: border-box; } .btn:hover, .buttons input:hover { background: #eef2f7; } .buttons { display: flex; justify-content: flex-end; gap: 10px; margin-top: 14px; } "
	".buttons input[name=Send] { background: #2563eb; border-color: #2563eb; color: #fff; } .buttons input[name=Send]:hover { background: #1d4ed8; } @media (max-width: 768px) { body { margin: 10px; } #myform { padding: 12px; } "
	".form-row { flex-direction: column; align-items: stretch; gap: 6px; margin: 6px 0; } .form-row label { flex: none; width: 100%%; } .form-row input, .form-row select { width: 100%%; } .form-row input, .form-row select, .buttons input, .btn { min-height: 44px; } "
	".form-row input[type=file] { min-height: 0; } .form-row.message-row label { padding-top: 0; } .form-row.meta-row label { flex: none; width: 100%%; } .form-row.meta-row select, .form-row.meta-row input { flex: 1 1 auto; max-width: none; } "
	".inline-action { white-space: normal; display: flex; flex-wrap: wrap; gap: 6px; width: 100%%; margin-left: 0; } .inline-action .btn { flex: 1 1 auto; min-width: 100px; } .buttons { justify-content: stretch; gap: 8px; } .buttons input { flex: 1 1 0; } }</style></head><body>"
	...
	"<div class='form-row meta-row'><label for='Type'>Type:</label><select id='Type' tabindex=1 size=1 name=Type><option value=P>P</option><option value=B>B</option><option value=T>T</option></select></div>"
...
static char CheckFormMsgPage[] = "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><meta content=\"text/html; charset=UTF-8\" http-equiv=\"content-type\">"
	...
	"<style>:root{--bg:#f5f7fa;--surface:#fff;--border:#cbd5e1;--text:#1f2937;}*{box-sizing:border-box;}body{font-family:-apple-system,BlinkMacSystemFont,\"Segoe UI\",Arial,sans-serif;margin:max(12px,env(safe-area-inset-left));"
	"background:var(--bg);color:var(--text);-webkit-font-smoothing:antialiased;}h3{text-align:center;margin:0 0 12px;}#myform{max-width:980px;margin:0 auto;background:var(--surface);border:1px solid #d1d5db;border-radius:8px;"
	"padding:14px;display:flex;flex-direction:column;}@supports(padding:max(0px)){body{margin:clamp(10px,3vw,20px);margin-left:max(clamp(10px,3vw,20px),env(safe-area-inset-left));margin-right:max(clamp(10px,3vw,20px),env(safe-area-inset-right));}}"
	".form-row{display:flex;align-items:center;gap:10px;margin:8px 0;}.form-row label{flex:0 0 clamp(70px,20%%,100px);font-weight:600;font-size:14px;}.form-row input,.form-row select{flex:1 1 auto;min-width:0;padding:8px 10px;border:1px solid var(--border);"
	"border-radius:6px;font-size:14px;touch-action:manipulation;}#To,#Subj{font-size:15px;padding:9px 10px;}.form-row textarea{flex:1 1 auto;width:100%%;min-height:360px;padding:10px;border:1px solid var(--border);border-radius:6px;line-height:1.4;resize:vertical;"
	"font-family:inherit;font-size:14px;touch-action:manipulation;}.form-row.message-row{align-items:flex-start;}.form-row.message-row label{padding-top:10px;}.form-row.meta-row label{flex:0 0 clamp(70px,20%%,100px);}.form-row.meta-row select{flex:0 0 120px;max-width:120px;}"
	".form-row.meta-row input{flex:0 1 260px;max-width:260px;}.btn,.buttons input{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:8px 14px;border:1px solid #94a3b8;border-radius:6px;background:var(--surface);color:var(--text);cursor:pointer;"
	"text-decoration:none;font-size:14px;touch-action:manipulation;}.btn:hover,.buttons input:hover{background:#eef2f7;}.buttons{display:flex;justify-content:flex-end;gap:10px;margin-top:14px;position:sticky;bottom:0;background:var(--surface);padding:10px 0;border-top:1px solid #e5e7eb;z-index:10;}"
	".buttons input[name=Send]{background:#2563eb;border-color:#2563eb;color:#fff;}.buttons input[name=Send]:hover{background:#1d4ed8;}@media(prefers-reduced-motion:reduce){*{animation-duration:0!important;transition-duration:0!important;}}@media(max-width:480px){body{margin:clamp(8px,2vw,12px);"
	"margin-left:max(clamp(8px,2vw,12px),env(safe-area-inset-left));margin-right:max(clamp(8px,2vw,12px),env(safe-area-inset-right));}#myform{padding:12px;border-radius:6px;}.form-row{flex-direction:column;align-items:stretch;gap:6px;margin:6px 0;}.form-row label{flex:none;width:100%%;}"
	".form-row input,.form-row select,.form-row textarea{width:100%%;min-height:48px;}.form-row.message-row{align-items:stretch;}.form-row.message-row label{padding-top:0;padding-bottom:4px;}.form-row.message-row textarea{min-height:300px;}.form-row.meta-row label{flex:none;width:100%%;}"
	".form-row.meta-row select,.form-row.meta-row input{flex:1 1 auto;max-width:none;min-height:48px;}.buttons{justify-content:stretch;gap:8px;padding:12px 0;}.buttons input{flex:1 1 0;min-height:48px;padding:12px 10px;}}</style></head><body>"
