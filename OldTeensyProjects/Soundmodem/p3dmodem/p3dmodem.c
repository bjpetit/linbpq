...
#define M_LOG10E 0.4329448190325182765f 
/* --------------------------------------------------------------------- */
...
static void compute_rxfilter(struct rxstate *rx)
{
	...
	rx->rxfiltlen = (rx->rxfiltlen + 3) & ~3;
        if (rx->rxfiltlen > RXFILTLEN) {
                WriteDebugLog(MLOG_WARNING, "demodp3d: input filter length too long\n");
                rx->rxfiltlen = RXFILTLEN;
        }
	WriteDebugLog(257, "p3d: rxfilter length %u, sampling rate %u\n", rx->rxfiltlen, rx->srate);
	tm = rx->rxfiltlen*RXFILTOVER*0.5;
		...
		f3 = f2 * sinc(t);
#if 0
		WriteDebugLog(258, "p3d: i %4u t %10g at %10g f1 %10g f3 %10g\n", i, t, at, f1, f3);
#endif
	...
	max1 = 0;
        ...
        if (logcheck(257)) {
                ...
                WriteDebugLog(257, "p3d: pulse energies: %s\n", buf+2);
        }
}
		...
		while ((phase - bufphase) < ((CRXBLOCKSIZE+10) << 16)) {
			...
			if (rxbufptr >= (512+2)*8) {
				if ((shreg & 0xffffffff) == SYNCWORD) {
					...
					WriteDebugLog(256, "p3ddemod: SYNC word: phase 0x%08x  finc: 0x%06x  (%d%+di)\n", phase, freqinc, r1.re, r1.im);
					continue;
				}
				rxbufptr++;
				  ...
				  int16_t *samp_re, int16_t *samp_im, cplxint_t *freqr)
{
	...
	*freqr = r4;
	if ((j & 0xffffffff) != SYNCWORD) {
		WriteDebugLog(256, "p3ddemod: finesync: phase 0x%08x word 0x%08x\n", phase, j);
		return 0;
	}
	r3.re = r3.im = 0;
	j = 0;
	...
	WriteDebugLog(256, "p3ddemod: finesync: phase 0x%08x val %6u  freqr %6d%+7di\n", phase, i, r4.re, r4.im);
	return i;
}
