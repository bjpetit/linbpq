...
BOOL GetNextARQFrame()
{
	...
	if (blnDISCRepeating)	// handle the repeating DISC reply 
	{
		...
		if (DebugLog) Debugprintf("Repeating DISC %d", intRepeatCount);
...
static UCHAR DataModes2000FM[] = {0x4C, 0x4A, 0x7C, 0x7A, 0x70, 0x72, 0x76};
...
void SendData()
{
	...
	case ISS:
		...
		else
		{
			...
			if (DebugLog) Debugprintf("[ARDOPprotocol.SendData]  Send IDLE with Repeat, Set ProtocolState=IDLE ");
  			return;
		}
	}
}
...
void ProcessRcvdARQFrame(UCHAR intFrameType, UCHAR * bytData, int DataLen, BOOL blnFrameDecodedOK)
{
	...
	if (DebugLog) Debugprintf("Time since received = %d", timeSinceDecoded);
...
//			ProcessUnconnectedConReqFrame(intFrameType, bytData);  //  displays data if not connnected.  
	...
	case IRS:
		...
		if (ARQState == IRSData || ARQState == IRSfromISS)  // Process Data or ConAck if ISS failed to receive ACK confirming bandwidth so ISS repeated ConAck
		{
			...
			{
				if (DebugLog) Debugprintf("[ARDOPprotocol.ProcessRcvdARQFrame]  IDLE received in ProtocolState %s substate %s", ARDOPStates[ProtocolState], ARQSubStates[ARQState]);
				{
					blnEnbARQRpt = FALSE; // setup for no repeats
					if (AutoBreak && bytDataToSendLength > 0)
					{
						// keep BREAK Repeats fairly short (preliminary value 1 - 3 seconds)
						intFrameRepeatInterval = ComputeInterFrameInterval(6000 + rand() % 2000);
		...
		case IRStoISS: // In this state answer any data frame with a BREAK. If ACK received go to Protocol State ISS
			...
			if (IsDataFrame(intFrameType) || intFrameType == IDLEFRAME)
			{
				if (DebugLog) Debugprintf("[ARDOPprotocol.ProcessRcvdARQFrame] ProtocolState = IRStoISS, substate = %s  Data or IDLE received, Answer with BREAK", ARQSubStates[ARQState]);
 			
				intFrameRepeatInterval = ComputeInterFrameInterval(6000 + rand() % 1000);  //keep BREAK Repeats fairly short (preliminary value 1 - 2 seconds)
 				blnEnbARQRpt = TRUE;	// setup for repeats until changeover
		...
		case IDLE:			// The state where the ISS has no data to send and is looking for a BREAK from the IRS
			...
			{
				if (DebugLog) Debugprintf("[ARDOPprotocol.ProcessedRcvdARQFrame] Protocol state IDLE, ACK Received with Data to send. Go to ISS Data state.");
			...
			if (intFrameType == BREAK)
			{
				...
				if (DebugLog) Debugprintf("[ARDOPprotocol.ProcessRcvdARQFrame] BREAK Rcvd from IDLE, Go to IRS, Substate IRSfromISS");
	...
	case ISS:
		...
		if (ARQState == ISSData)
		{
			...
			if (intFrameType >= 0xE0)	// if ACK
			{
				...
				SendData(FALSE);		// Send new data from outbound queue and set up repeats
...
void Break()
{
	...
	if (ProtocolState == IRS)
	{
		...
		if (DebugLog) Debugprintf("[ARDOPprotocol.Break] %d bytes to send in ProtocolState: %s : Send BREAK,  New protocol state=IRStoISS (Rule 3.3)", bytDataToSendLength, ARDOPStates[ProtocolState]);
