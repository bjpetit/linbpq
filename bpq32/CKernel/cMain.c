...
BPQVECSTRUC BPQHOSTVECTOR[BPQHOSTSTREAMS + 4] = {0};

BPQVECSTRUC * TELNETMONVECPTR = &BPQHOSTVECTOR[BPQHOSTSTREAMS];
BPQVECSTRUC * APRSMONVECPTR = &BPQHOSTVECTOR[BPQHOSTSTREAMS + 1];
BPQVECSTRUC * IPHOSTVECTORPTR = &BPQHOSTVECTOR[BPQHOSTSTREAMS + 2];
...
int BPQTRACE(UINT * Msg, BOOL TOAPRS)
{
	...
	UINT * Buffer;
	int i = BPQHOSTSTREAMS + 1;		// Include Telnet Stream
