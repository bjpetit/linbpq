...
VOID SENDNEXTNODESFRAGMENT()
{
	...
	if (DEST == 0)
	{
		...
		while (PORT->PORTQUALITY == 0 || PORT->TXPORT)		// Don't send NODES to Shared TX Port
		{
...
VOID L3LINKCLOSED(struct _LINKTABLE * LINK)
{
	...
	if (ROUTE)
	{
		...
		CLEARACTIVEROUTE(ROUTE);		// CLEAR ASSOCIATED DEST ENTRIES
	}
}
