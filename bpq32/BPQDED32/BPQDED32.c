...
//		Fix crash caused by appl polling streams that have been deallocated

//  Version 1.03 December 2007

//		Add MTX parmam to @S command string
//		Fix format of Digi string
//		Fix unsetting of Monitor bit in APPLFLAGS by I Command
...
SETMASK:
	...
	MOV	ESI,CURSOR
	MOV	DWORD PTR [ESI],20202020H	; SPACES ON END
...
MASKBAD:
	...
	strcpy(Errbuff,	"BPQDED32 Monitoring Params: MMASK ");
...
; SETTING UNPROTO ADDR - JUST ACK IT
	...
	OutputDebugString("Setting UNPROTO - Ignored\r\n");
	OutputDebugString(LINEBUFFER);

	_asm{

	JMP	JCOMM00			; SETTING UNPROTO ADDR - JUST ACK IT
...
ENABLEHOST:
	...
	OR	CL,MonEnabled	; Set monitor bit on first stream
NOTFIRST:
	CALL	NODE
...
;
		...
		strcpy(Errbuff,	"BPQDED32 Normal Mode CMD ");
		strcat(Errbuff,LINEBUFFER);
		strcat(Errbuff,"\n");
