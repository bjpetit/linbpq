//
//	IMP3 Suport Code for BPQ32 Switch
//
...
typedef struct _IMP3Node
{
	...
	struct _IMP3Neighbour * Neighbour;	// Adjacent Node
};
...
VOID SendNRRecordRoute(char * Call, int Stream);
VOID SendIMP3RIF(struct ROUTE * Route, UCHAR * Call, UCHAR * Alias, int Hops, int RTT);
...
TellIMP3LinkGone(struct ROUTE * Route)
{
	Route->SRTT = 0;
	Route->RTT = 0;
	Route->BCTimer = 0;
...
VOID ProcessRTTReply(struct ROUTE * Route, struct _L3MESSAGE * Buff)
{
	...
	Route->Status = 1;			// Route is active
	...
	if (Route->SRTT == 0)
		...
		Route->SRTT = ((Route->SRTT * 80)/100) + (RTT/20);
}

VOID ProcessIMP3RIF(struct ROUTE * Route, UCHAR * ptr1, int msglen, int Port)
{
	char axcall[7];
	int hops;
	short rtt;
...
VOID ProcessRTTMsg(struct ROUTE * Route, struct _L3MESSAGE * Buff, int Len, int Port)
{
	...
	else
	{
		...
		Route->IMP3Node = TRUE; // Must be!

		if (Route->Status == 0)
		{
			// Just Starting - Send our RTT
...
//		if (Route->NeighbourSRTT && Route->SRTT)
//			SendIMP3RIF(Route, "G8BPQ", "BPQPMS", 1, 100);
...
VOID SendIMP3RIF(struct ROUTE * Route, UCHAR * Call, UCHAR * Alias, int Hops, int RTT)
{
	...
	RIFLen = BuildRIF(&Msg.L3MSG.L3SRCE[totLen], Call, Alias, Hops, RTT);
	totLen += RIFLen;
...
UCHAR * DisplayIMP3RIF(UCHAR * ptr1, UCHAR * ptr2, int msglen)
{
	char call[10];
	int calllen;
	int hops;
	...
	ptr2+=wsprintf(ptr2, " IMP3 RIF:\r  Call Alias Hops RTT\r");
...
VOID NRRecordRoute(char * Buff, int Len)
{
	...
	{
		// Steeam is encoded in ID field

		int Stream = Buff[50] - 28;
...
{	
	...
	memcpy(Msg.L3MSG.L3DEST, AXCall, 7);
	memcpy(Msg.L3MSG.L3SRCE, MYCALL, 7);
