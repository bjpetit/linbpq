...
//					if (!CONNECTING[port])
//					{
//						i=wsprintf(ErrMsg, "MPSK Connection lost for BPQ Port %d\r\n", port);
		...
		for (Stream = 0; Stream <= TNC->AGWInfo->MaxSessions; Stream++)
		{
			...
			if (STREAM->Connecting)
			{
				...
				if (STREAM->Connecting == 0)
				{
					...
					if (buffptr)
					{
						buffptr[1] = wsprintf((UCHAR *)&buffptr[2], "MPSK} Failure with %s\r", STREAM->RemoteCall);
			...
			if (_memicmp(&buff[8], "INUSE?", 6) == 0)
			{
				...
				while(s <= TNC->AGWInfo->MaxSessions)
				{
					...
					{		
						...
						{
							buffptr[1] = wsprintf((UCHAR *)&buffptr[2], "MPSK} Error - In use\r");
							C_Q_ADD(&STREAM->PACTORtoBPQ_Q, buffptr);
							return 1;							// Busy
						}
					}
				...
				buffptr[1] = wsprintf((UCHAR *)&buffptr[2], "MPSK} Ok - Not in use\r");
				C_Q_ADD(&STREAM->PACTORtoBPQ_Q, buffptr);
...
UINT WINAPI MPSKExtInit(EXTPORTDATA * PortEntry)
{
	...
	i=wsprintf(Msg,"MPSK Host %s Port %d \n",
...
static VOID ConnecttoMPSKThread(port)
{
	...
	{
		i=wsprintf(Msg, "Socket Failed for MPSK socket - error code = %d\n", WSAGetLastError());
		WritetoConsole(Msg);
	...
	else
	{
		...
		{
			err=WSAGetLastError();
   			i=wsprintf(Msg, "Connect Failed for MPSK socket - error code = %d\n", err);
			WritetoConsole(Msg);
...
static int ProcessReceivedData(int port)
{
	...
	if (bytes == SOCKET_ERROR)
	{
//		i=wsprintf(ErrMsg, "Read Failed for MPSK socket - error code = %d\r\n", WSAGetLastError());
//		WritetoConsole(ErrMsg);
	...
	if (bytes == 0)
	{
		...
		i=wsprintf(ErrMsg, "MPSK Connection closed for BPQ Port %d\n", port);
		WritetoConsole(ErrMsg);
	...
	//	goto DataLoop;
		...
		if (cptr)			// Have a connection
		{
			...
			if (STREAM->Connecting)
			{
				...
				if (buffptr)
				{
					buffptr[1]  = wsprintf((UCHAR *)&buffptr[2], "*** Connected to %s\r", CallFrom);
					C_Q_ADD(&STREAM->PACTORtoBPQ_Q, buffptr);
				}				
			}
			else
