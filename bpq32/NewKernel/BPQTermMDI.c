...
VOID CALLBACK SetupTermSessions(HWND hwnd, UINT  uMsg, UINT  idEvent,  DWORD  dwTime)
{
	...
	wsprintf(Key, "SOFTWARE\\G8BPQ\\BPQ32\\BPQTermMDI");
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	wsprintf(Title,"BPQTerminal Version %s", VersionString);
	...
	case WM_SYSCOMMAND:
		...
		case WM_DESTROY:
			...
			for (Cinfo = ConsHeader; Cinfo; Cinfo = Cinfo->next)
			{
				...
				wsprintf(Size, "%d,%d,%d,%d", Cinfo->Left, Cinfo->Height, Cinfo->Top, Cinfo->Width);
				wsprintf(Item, "SessSize%d", i);
			...
			wsprintf(Size, "%d,%d,%d,%d", MonWindow.Left, MonWindow.Height, MonWindow.Top, MonWindow.Width);
...
DoStateChange(int Stream)
{
	...
	if (change == 1)
	{
		...
		else
		{
			...
			wsprintf(Title,"Stream %d - Disconnected", Stream);
			SetWindowText(Cinfo->hConsole,Title);
...
BOOL OpenMonitorLogfile()
{
	...
	if (BPQDirectory[0] == 0)
		wsprintf(FN,"BPQTerm.log");
	else
		wsprintf(FN,"%s\\BPQTerm.log", BPQDirectory);
