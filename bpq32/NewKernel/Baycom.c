...
UINT WINAPI BaycomExtInit(struct PORTCONTROL *  PortEntry)
{
	char msg[255];
	char szPort[15];
	...
	wsprintf(msg,"Baycom Serial Modem Port %d", Port);
	WritetoConsole(msg);
	...
	wsprintf(szPort, "\\\\.\\COM%d", PortEntry->IOBASE);
	...
	if (hDevice == (HANDLE) -1 )
	{
		wsprintf(msg, "COM%d could not be opened ", PortEntry->IOBASE);
		WritetoConsole(msg);
		Baycom->hDevice = NULL;
	}
	else
	{
		...
		{
			wsprintf(msg, " Baycom Driver not found");
			CloseHandle(hDevice);
			Baycom->hDevice = NULL;
			WritetoConsole(msg);
		}
