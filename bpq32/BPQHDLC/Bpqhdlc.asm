...
INT0PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 2
INT1PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 3
INT2PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 2
INT3PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 3
INT4PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 4
INT5PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 5
INT6PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 6
INT7PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 7
INT8PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 8
INT9PORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 9
INTAPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 10
INTBPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 11
INTCPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 12
INTDPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 13
INTEPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 14
INTFPORTBASE	DD	0		; PORT CONTROL TABLE ENTRY FOR INT 15
	
INT2HAPPENED	DB	1		; INTERRUPT HAPPENED RECENTLY FLAG
INT3HAPPENED	DB	1
INT4HAPPENED	DB	1
INT5HAPPENED	DB	1
INT6HAPPENED	DB	1
INT7HAPPENED	DB	1
INT8HAPPENED	DB	1		; INTERRUPT HAPPENED RECENTLY FLAG
INT9HAPPENED	DB	1
INTAHAPPENED	DB	1
INTBHAPPENED	DB	1
INTCHAPPENED	DB	1
...
;
	PUSHAD
	Debug_Printf	"hdlc98 Channel Definition Problem"
	POPAD
...
BAYCOMINIT:
	...
	PUSHAD
	movzx eax, IOBASE[EBX]
	Debug_Printf	"hdlc98 USCCInit IOADDR  %x", <eax>
	POPAD
	...
	Debug_Printf	"hdlc98 USCCInit SCC BASE ADDR  %x", <eax>
	POPAD
...
;
	...
	PUSH	EBX
	CALL	PORTINTERRUPT[EBX]
	POP		EBX
;
;	IF SHARED INTERRUPT, CALL OTHERS, THEN LOOP BACK AND CALL
;	THEM ALL AGAIN
;
	CMP	INTCHAIN[EBX],0
	JE	ENDINT			; NOT SHARED

	PUSH	EBX
	
	MOV		EBX,INTCHAIN[EBX]
...
;	See if already in use
	...
	MOV	EDI,OFFSET INT2PORTBASE
	ADD	EDI,EAX
;
;	SEE IF LEVEL ALREADY IN USE
;
	CMP	DWORD PTR [EDI],0
	JE	NOTINUSE
;
;	LEVEL IN USE - CHAIN THIS PORT ENTRY TO FIRST
;
	PUSHAD
	movzx eax, INTLEVEL[EBX]
	Debug_Printf	"hdlc98 Interupt %x already in use - chaining entry", <eax>
	POPAD

	MOV	EDI,[EDI]		; GET FIRST PORT ENTRY
...
FINDENDOFCHAIN:
	...
	MOV	EDI,INTCHAIN[EDI]
	JMP	FINDENDOFCHAIN

ENDOFCHAIN:

	MOV	INTCHAIN[EDI],EBX
	RET

NOTINUSE:

	MOV	DWORD PTR [EDI],EBX		; SAVE PORT CONTROL ADDRESS
	...
	mov	eax, offset INTMUX
	mov	VID_Hw_Int_Proc[EDI],EAX

	mov	VID_Options[EDI],VPICD_OPT_REF_DATA
	MOV	VID_Hw_Int_Ref[EDI],ecx				; pointer to first card on this level
