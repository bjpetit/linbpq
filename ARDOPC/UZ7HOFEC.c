//	Retimes to support UZ7HO's PSK FEC Mode
...
//	while (intFilteredMixedSamplesLength > (960 + 120))		// Experimental UZ7HO FSK Detect
		...
		for (i = x; i < y; i++)
		{
			...
			Used = Demod1CarPSKUZ7HO(Samples, Start + i, 0);
			intPhasesLen -= 8; //intPSKMode;
			CarFreq = carriers[1];
	
			Demod1CarPSKUZ7HO(Samples, Start + i, 1);
			intPhasesLen -= 8; //intPSKMode;
			CarFreq = carriers[2];

			Demod1CarPSKUZ7HO(Samples, Start + i, 2);
			intPhasesLen -= 8; //intPSKMode;
			CarFreq = carriers[3];

			Used = Demod1CarPSKUZ7HO(Samples, Start + i, 3);

			//	Decode Phases. 0 = 180 shift 1 = no shift
			for (j = 0; j < 8; j++)
			{
				if (intPhases[0][j] < 1572 && intPhases[0][j] > -1572)
					x1[j] = '1';
				else
					x1[j] = '0';
				if (intPhases[1][j] < 1572 && intPhases[1][j] > -1572)
					x2[j] = '1';
				else
					x2[j] = '0';
				if (intPhases[2][j] < 1572 && intPhases[2][j] > -1572)
					x3[j] = '1';
				else
					x3[j] = '0';
				if (intPhases[3][j] < 1572 && intPhases[3][j] > -1572)
					x4[j] = '1';
				else
					x4[j] = '0';
			}
...
//					intPhases[0][4], intPhases[0][5], intPhases[0][6], intPhases[0][7]);
			...
			if (UZ7HOIndex == -1)
			{
				if (strcmp(x1, "11111111") == 0 &&
					strcmp(x4, "11111111") == 0 &&
