...
#define SAMPLE_RATE     12000   // 48kHz
#define CHANNELS        1               // 1 = mono, 2 = stereo
#define SAMPLES_PER_BLOCK 1200
// *******************************************

// the ADC DMA saves the incoming ADC samples into these 2 buffers
 uint16_t       ADC_Buffer[2][SAMPLES_PER_BLOCK * 2] = {0};     // stereo capture buffers
volatile int	adc_buffer_mem = -1;

// the executives ADC buffer - our own buffer
 int16_t        audioInputBuffer[SAMPLES_PER_BLOCK * CHANNELS] = {0};
 float          audioInputBuffer_float[SAMPLES_PER_BLOCK * CHANNELS] = {0};

 int16_t		DAC_Buffer[2][SAMPLES_PER_BLOCK * 2] = {0};                     // stereo playback buffers
volatile int 	dac_block = -1;

// *******************************************
// ADC1 channel 11 on pin PC1 with DMA2 Stream 0 configuration using double buffering
// ADC2 channel 12 on pin PC2 with DMA2 Stream 0 configuration using double buffering
//
// System clock ...... 168MHz
// APB1 .............. 42MHz
// APB2 .............. 84MHz
// ADC clock ......... APB2 / 4 .... 21.0MHz
// ADC 12-bit mode ... 12 cycles
// Sampling time ..... 480 cycles
//
// (12 + 480) / 21000000 = 23.428us (42.683ksps)
//
// BUT ...
//
// We are now using Timer2 to clock the ADCs at 48kHz
...
void Config_ADC_DMA(void)
{
	...
	ADC_Cmd(ADC2, DISABLE);
        ...
        // Configure ADC1-Channel-11 (PC1) ADC2-Channel-12 (PC2) pins as analog inputs
...
//      ADC_CommonInitStructure.ADC_Mode = ADC_Mode_Independent;
        ADC_CommonInitStructure.ADC_Mode = ADC_DualMode_RegSimult;     // simultaneous multi ADC sampling
...
//      ADC_RegularChannelConfig(ADC2, ADC_Channel_12, 1, ADC_SampleTime_480Cycles);    // PC2 .. 3, 15, 28, 56, 84, 112, 144 or 480
        ADC_RegularChannelConfig(ADC1, ADC_Channel_11, 1, ADC_SampleTime_112Cycles);            // PC1 .. 3, 15, 28, 56, 84, 112, 144 or 480
        ADC_RegularChannelConfig(ADC2, ADC_Channel_12, 1, ADC_SampleTime_112Cycles);            // PC2 .. 3, 15, 28, 56, 84, 112, 144 or 480
...
//      DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&ADC2->DR;
        DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&ADC->CDR;                         // multi ADC address
        DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&ADC_Buffer[0];                       // buffer address
        DMA_InitStructure.DMA_BufferSize = SAMPLES_PER_BLOCK * 2;                                       // total number of buffer elements - NOT bytes
...
//      NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);
        ...
        NVIC_InitStructure.NVIC_IRQChannelSubPriority = 6;                      // lower number = higher priority
        NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;                         // enable interrupt
        NVIC_Init(&NVIC_InitStructure);

        DMA_ITConfig(DMA2_Stream0, DMA_IT_TC, ENABLE);                          // transfer complete interrupt
//      DMA_ITConfig(DMA2_Stream0, DMA_IT_HT, ENABLE);                          // half transfer complete interrupt
        ...
        TIM_TimeBaseStructure.TIM_Period = (170000000 / SAMPLE_RATE) - 1;        // 12KHz, from 170MHz TIM2CLK (ie APB1 = HCLK / 4, TIM2CLK = HCLK / 2)
...
//              DMA_ClearFlag(DMA2_Stream0, DMA_FLAG_TCIF0);
                ...
                if (adc_buffer_mem < 0) // the exec is ready for new samples
